      ...
      <nav className="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold text-amber-400">机器人详情</span>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <div className="flex items-start gap-6">
            <div className="w-24 h-24 bg-slate-700 rounded-xl flex items-center justify-center text-4xl">
              🤖
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-2xl font-bold text-white">{bot.nickname}</h1>
                <span className={`font-semibold ${statusColors[bot.status]}`}>
                  ● {statusTexts[bot.status]}
                </span>
              </div>
              <p className="text-slate-400 mb-4">QQ: {bot.bot_id}</p>
              {bot.description && (
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-700">
            <div>
              <p className="text-slate-500 text-sm">版本</p>
              <p className="text-white">{bot.version || '-'}</p>
            </div>
            <div>
              <p className="text-slate-500 text-sm">主人</p>
              <p className="text-white">QQ: {bot.master_qq}</p>
            </div>
            <div>
              <p className="text-slate-500 text-sm">最后在线</p>
              <p className="text-white">
                {bot.last_seen ? new Date(bot.last_seen).toLocaleString('zh-CN') : '-'}
              </p>
            </div>
            <div>
              <p className="text-slate-500 text-sm">添加时间</p>
